...
class SmsDetectorService {
  ...
  SmsDetectorService({FinanceService? financeService})
  ...
  static const _prefKeyEnabled = 'sms_detector_enabled';
  static const _prefKeySeenIds = 'sms_detector_seen_ids';
  ...
  Future<void> setEnabled(bool enabled) async {
    ...
    if (enabled) {
      ...
      if (status.isGranted) await scanAndStore();
    }
  }
  ...
  Future<int> scanAndStore() async {
    ...
    try {
      ...
      return await processMessages(messages, prefs, userId);
    } catch (e) {
  ...
  @visibleForTesting
  Future<int> processMessages(
    List<SmsMessage> messages,
    SharedPreferences prefs,
    String userId,
  ) async {
    final seenIds = prefs.getStringList(_prefKeySeenIds) ?? [];
    int savedCount = 0;

    for (final msg in messages) {
      // Duplicate check: skip messages already saved.
      final sourceId = msg.id?.toString();
      if (sourceId != null && seenIds.contains(sourceId)) continue;
    ...
    if (savedCount > 0) {
      await prefs.setStringList(_prefKeySeenIds, seenIds);
    }
  ...
  String detectCategory(String body) {
    ...
    if (b.contains('uber eats') ||
        b.contains('food') ||
        b.contains('restaurant') ||
        b.contains('grocery') ||
        b.contains('supermarket')) {
      return 'Food';
    }
    if (b.contains('uber') ||
        b.contains('pickme') ||
        b.contains('transport') ||
        b.contains('fuel') ||
        b.contains('petrol')) {
      return 'Transportation';
    }
    if (b.contains('electricity') ||
        b.contains('water bill') ||
        b.contains('internet bill') ||
        b.contains('utility') ||
        b.contains('dialog') ||
        b.contains('hutch') ||
        b.contains('slt')) {
